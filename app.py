...
def create_app():
    ...
    def inject_i18n():
        ...
        # 현재 URL의 다른 쿼리 스트링은 유지하면서 lang 매개변수만 교체하는 헬퍼
        ...
        return dict(_=translate_helper, current_lang=valid_lang, google_client_id=Config.GOOGLE_CLIENT_ID, change_lang=change_lang_url)
    ...
    def page_not_found(e):
        lang = request.args.get('lang', 'kor')
        valid_lang = get_validated_lang(lang)
        return render_template('error.html', status_code=404, current_lang=valid_lang), 404

    @app.errorhandler(500)
    def internal_server_error(e):
        lang = request.args.get('lang', 'kor')
        valid_lang = get_validated_lang(lang)
        return render_template('error.html', status_code=500, current_lang=valid_lang), 500
