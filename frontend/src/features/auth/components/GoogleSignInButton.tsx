...
export function GoogleSignInButton({ onCredential, disabled = false }: Props) {
  ...
  useEffect(() => {
    ...
    void loadGoogleScript().then(() => {
      ...
      if (!clientId) { setError('Google Client ID가 설정되지 않았습니다.'); return; }
      ...
      window.google.accounts.id.renderButton(container.current, { theme: 'outline', size: 'large', type: 'standard', shape: 'pill', text: 'signin', logo_alignment: 'left' });
    }).catch((loadError) => { if (active) setError(loadError instanceof Error ? loadError.message : 'Google 로그인을 준비하지 못했습니다.'); });
    return () => { active = false; };
  }, [onCredential]);
