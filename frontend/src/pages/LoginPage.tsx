...
export function LoginPage() {
  ...
  return <section className="mx-auto flex min-h-[calc(100vh-64px)] max-w-5xl items-center justify-center px-4 py-12 sm:px-6"><div className="w-full max-w-md rounded-[2rem] border border-slate-200 bg-white p-7 text-center shadow-xl shadow-slate-200/60 sm:p-9"><p className="eyebrow">Frog Lab · LinkSuwon</p><h1 className="mt-3 text-3xl font-black text-slate-900">수원 여행을 이어서 기록하세요</h1><p className="mt-4 text-sm leading-6 text-slate-500">Google 계정으로 로그인하면 여행 기록과 동기화 데이터를 안전하게 이어갈 수 있습니다.</p><div className="mt-8 flex justify-center"><GoogleSignInButton onCredential={(credential) => void handleCredential(credential)} disabled={loading} /></div>{loading && <p className="mt-4 text-sm text-suwon">로그인 처리 중...</p>}{error && <p className="mt-4 rounded-xl bg-red-50 p-3 text-sm text-red-700">{error}</p>}<p className="mt-8 text-xs leading-5 text-slate-400">Google credential은 브라우저에 저장하지 않고 백엔드에서 검증합니다.</p></div></section>;
}
